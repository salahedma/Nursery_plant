...
--seedlings (add seedling_name nvarchar(20) row 3....) and (add seedling_imported_country nvarchar(20).......)
...
--السبب هو إنه غالبًا ما نبحث عن المواسم حسب فترة زمنية
create index season_manage_dates_index on season_manage (start_date, end_date);

-- الباركود: مهم لفهرسته لتسريع عمليات البحث والتتبع
create index seedlings_barcode_index on seedlings (barcode);

-- فهرسة حالة النبات تساعد في تسريع الاستعلامات التي تعتمد على حالة النبات
create index seedlings_status_index on seedlings (status);

-- يستخدم seedling_id و purchase_date في استعلامات تتعلق بفواتير الشراء أو بناءً على تاريخ الشراء
...
-- يستخدم لتتبع المخزون ولإيجاد آخر التحديثات في مخزون النباتات
...
-- يربط العمليات اللوجستية مع النباتات ويستخدم التاريخ لتسريع الاستعلامات الزمنية
